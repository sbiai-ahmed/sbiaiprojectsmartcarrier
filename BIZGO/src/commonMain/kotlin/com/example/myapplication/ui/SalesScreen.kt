...
fun SalesScreen(
    ...
    Scaffold(
        ...
        floatingActionButton = {
            // الموظف والمدير كلاهما يمكنهما البيع
            FloatingActionButton(
        ...
        Column(modifier = Modifier.fillMaxSize().padding(padding)) {
            ...
            Box(modifier = Modifier.fillMaxSize().padding(16.dp)) {
                Column(modifier = Modifier.fillMaxSize()) {
                    if (selectedTab == 1 && isAdmin) { // عرض الأرباح للمدير فقط
                            ...
                            LazyColumn(
                                ...
                                items(filteredSales, key = { it.saleId }) { sale ->
                                    Box(modifier = Modifier.animateItem()) {
                                        SaleRecordItem(
                                            sale = sale,
                                            onEdit = { if (isAdmin) editingSale = sale },
                                            onDelete = {
                                                if (isAdmin) {
    ...
    Card(
        ...
        Row(modifier = Modifier.padding(12.dp), verticalAlignment = Alignment.CenterVertically) {
            ...
            Row(verticalAlignment = Alignment.CenterVertically) {
                ...
                if (isAdmin) { // التعديل والحذف للمدير فقط في سجل المبيعات
